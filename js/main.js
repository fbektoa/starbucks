...
//포커스가 들어가면 class focused'추가
searchInputEl.addEventListener("focus", function () {
  searchEl.classList.add("focused");
  searchInputEl.setAttribute("placeholder", "통합검색");
});

//포커스가 나가면 class focused'제거
...
menuItems.forEach(item => {
  const contents = item.querySelector('.item__contents');

  // 마우스를 올렸을 때 (펼쳐짐)
  item.addEventListener('mouseover', () => {
    // 실제 서브 메뉴의 높이를 계산해서 넣어주거나, 고정값(예: 350px)을 줍니다.
    contents.style.height = '350px';
  });
